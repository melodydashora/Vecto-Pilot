...
| Response (200) | `{ success, voice, notification, decision:'ACCEPT'\|'REJECT'\|'NO DATA', reason, notices:[], response_time_ms }` — speak `voice`, show `notification`, branch on `decision` if you add logic. (The share auto-reject reply omits `notices`.) `duplicate:true` is added when the same card was re-sent within 60 s (see Re-sends). |
| Re-sends | **Safe when the payload is identical.** A network-level retry, a share-sheet re-run of the same screenshot, or the same saved image/text sent again within 60 s under the same rules: the server recognizes it and replays the first answer — no second analysis, no second row on the Offer Analyzer page (2026-08-17). Note: a Shortcut *re-run* takes a **new** screenshot, so a double Back Tap is usually two different payloads (two analyses); a rules edit always gets a fresh analysis. |
