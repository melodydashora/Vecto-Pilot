...
| `migrations/20260505_coach_offer_decisions.sql:12` | "See claudeMd Rule 8" (no such rule since the boot-sequence rewrite) | **REVERTED** — the boot runner checksums applied migrations (`[MIGRATIONS] ⚠️ checksum drift … NOT re-running`); applied history is immutable, even comments. The stale pointer stays; this row is the correction: write-access ownership = the Coach's action tags (chat.js executeActions) |
