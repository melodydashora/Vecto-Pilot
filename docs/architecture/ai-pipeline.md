...
- `role` (string): The specific role key defined in `model-registry.js`. Follows the `{TABLE}_{FUNCTION}` convention (e.g., `'BRIEFING_WEATHER'`, `'BRIEFING_TRAFFIC'`, `'BRIEFING_HOLIDAY'`). The role configuration dictates the model selection, **reasoning effort (`thinkingLevel`)**, token limits, and enabled tools (e.g., Google Search).
...
- **Provider Abstraction:** Automatically routes to the correct adapter (`openai`, `anthropic`, `google`, `vertex`) based on the role configuration, handling provider-specific features like **thinking levels** (reasoning) and multimodal inputs.