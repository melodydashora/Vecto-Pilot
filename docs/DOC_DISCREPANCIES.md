...
### REPO HYGIENE — Dead Code / Reintroduction Hazard (P1)

| ID | Location | Issue | Root Cause | Status |
|----|----------|-------|------------|--------|
| D-110 | `server/jobs/event-sync-job.js` (whole file) | Orphaned dead code that contradicts Rule 11. `startEventSyncJob` was removed from the bootstrap on 2026-02-17 (`gateway-server.js:113` carries the removal comment; per-snapshot discovery via `pipelines/events.js` replaced it). But the file still physically exists, still exports `startEventSyncJob()` (line 160), and has a CLI self-run guard (line 210) so it can be resurrected with `node server/jobs/event-sync-job.js`. Its own docstring (line 8) still claims "Started by bootstrap/workers.js in mono mode" — verified false: a repo-wide grep for `event-sync-job`/`startEventSyncJob` returns only this file + the removal comment in `gateway-server.js`; nothing imports it. This is a textbook Rule 11 reintroduction hazard: a future vibe-coding session that greps for "event sync" finds a clean, runnable, *self-describing-as-wired* module and re-enables the exact background-fetch pattern that was deliberately killed for drifting from snapshot context. **Grep finds the corpse; only Rule 11 + this entry explain why it must stay dead.** Surfaced 2026-06-03 during the memory-table architecture eval requested by Melody. Fix options: (a) delete the file outright (Rule 11 says the pattern is dead) — architecture call, needs Melody's go-ahead per Rule 16; (b) if kept for reference, add a `@deprecated DEAD — see Rule 11, do not wire up` banner to the docstring and remove the CLI self-run guard so it can't be executed. | Stale artifact left behind when the call site was removed but the module wasn't; module's own docstring was never updated | PENDING |
