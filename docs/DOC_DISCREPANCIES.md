...
| D-104 | `docs/architecture/` | **Case-collision duplicate doc files**: `DECISIONS.md` ↔ `decisions.md` (identical 16458 bytes), `LOCATION.md` ↔ `Location.md` (identical 46352 bytes), `STANDARDS.md` (8927 bytes, canonical) ↔ `standards.md` (8849 bytes, divergent). Two filenames differing only by case both exist in git on case-sensitive filesystems but are aliases on macOS-checked-out clones — edits to one don't propagate to the other, creating silent doctrine fork. Surfaced 2026-05-03 during Workstream 1 Split-Brain Governance Audit when the new `pending-md-drift` lint flagged stale references in lowercase variants. The lint currently whitelists the 3 lowercase variants per the audit decision to defer resolution. Resolution: pick one canonical case per file, delete the other, remove from the whitelist in `scripts/check-standards.js:checkPendingMdDrift()`. Tracked in `claude_memory` for next-session resolution. | PENDING |
