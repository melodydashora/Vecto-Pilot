...
const AGENT_TABLE = "agent_memory";
...
export async function getEnhancedProjectContext(options = {}) {
  ...
  try {
    ...
    console.warn('[Agent Enhanced Context] Failed to load recent snapshots:', err.message);
  }
  ...
  try {
    ...
    console.warn('[Agent Enhanced Context] Failed to load recent actions:', err.message);
  }
  ...
  try {
    const prefs = await memoryQuery({ 
      table: AGENT_TABLE, 
      scope: "agent_preferences", 
      userId: null,
      limit: 50 
    });
    context.agentPreferences = Object.fromEntries(
      prefs.map(p => [p.key, p.content])
    );
  } catch (err) {
    console.warn('[Agent Enhanced Context] Failed to load preferences:', err.message);
  }
  ...
  try {
    ...
    console.warn('[Agent Enhanced Context] Failed to load session history:', err.message);
  }
  ...
  try {
    ...
    console.warn('[Agent Enhanced Context] Failed to load project state:', err.message);
  }
  ...
  try {
    ...
    console.warn('[Agent Enhanced Context] Failed to load conversation history:', err.message);
  }
  ...
  try {
    const response = await fetch("https://api.anthropic.com/v1/messages", {
      ...
      body: JSON.stringify({
        ...
        max_tokens: 4096,
        system: "You are Agent, a technical assistant with deep development knowledge. Provide accurate, up-to-date information with citations from web search results.",
        tools: [
          {
...
export async function storeAgentMemory(title, content, metadata = {}, ttlDays = 730) {
  try {
    ...
    await pool.query(
      `INSERT INTO ${AGENT_TABLE} (session_id, entry_type, title, content, metadata, expires_at)
       VALUES ($1, $2, $3, $4, $5, $6)`,
      [
        'agent',
        'context',
    ...
    console.warn('[Agent Enhanced Context] Failed to store agent memory:', err.message);
    return false;
  }
}
...
export async function getAgentMemory(userId = null, limit = 50) {
  try {
    ...
    console.warn('[Agent Enhanced Context] Failed to load agent memory:', err.message);
    return [];
  }
}
