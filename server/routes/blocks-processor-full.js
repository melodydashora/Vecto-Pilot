...
export class BlocksProcessorError extends Error {
  constructor(message, httpStatus = 500, details = undefined) {
    super(message);
    this.name = 'BlocksProcessorError';
 ...
 * @returns {Promise<Object>} - EXACT same JSON shape the old handler returned
 */
export async function processBlocksRequestCore({ body, headers }) {
  const startTime = Date.now();
  const correlationId = headers['x-correlation-id'] || randomUUID();
  
  // sendOnce adapter for compatibility
  const sendOnce = (code, body) => {
    if (code === 200) {
      return body;
    } else {
      throw new BlocksProcessorError(JSON.stringify(body), code);
    }
  };
  ...
  try {
    ...
    console.error(`❌ [${correlationId}] Processor error:`, error);
    if (error instanceof BlocksProcessorError) {
      throw error;
    }
    throw new BlocksProcessorError(error.message, 500, { originalError: error.name });
  }
}
