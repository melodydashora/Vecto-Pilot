...
router.post('/', async (req, res) => {
  ...
  try {
    ...
    // Load strategy from DB (already generated by GPT Strategist - NOT NULL triggered)
    console.log(`🎯 [${correlationId}] Loading strategy for snapshot ${snapshotId}...`);
    const [strategyRow] = await db.select().from(strategies)
      .where(eq(strategies.snapshot_id, snapshotId))
      .limit(1);
    
    if (!strategyRow || strategyRow.status !== 'ok') {
      return sendOnce(400, {
        error: 'strategy_not_ready',
        message: 'Strategy not yet generated or failed',
        status: strategyRow?.status || 'missing'
      });
    }

    const strategy = strategyRow.strategy;
    const driverAddress = fullSnapshot.formatted_address || fullSnapshot.resolved_address || `${lat}, ${lng}`;
