...
router.post('/', async (req, res) => {
  ...
  try {
    const { userId = 'demo', strategy, driverAddress } = req.body;

    // Require strategy from DB (already generated by Strategist/GPT)
    if (!strategy) {
      return sendOnce(400, {
        error: 'strategy_required',
        message: 'Strategy required (from strategy DB after enrichment)'
      });
    }

    // Require driver's precise resolved address
    if (!driverAddress) {
      return sendOnce(400, {
        error: 'driver_address_required',
        message: 'Driver precise address required (resolved from GPS coords)'
      });
    }

    console.log(`🎯 [${correlationId}] PLANNER REQUEST: strategy="${strategy.slice(0, 50)}..." driverAddress="${driverAddress}"`);

    // Build minimal context for planner - it will return coordinates for venues
    const snapshotId = randomUUID();
    const fullSnapshot = {
      snapshot_id: snapshotId,
      resolved_address: driverAddress,
      // Planner doesn't need exact coords - it generates venue coords which Google APIs will enrich
      lat: null, 
      lng: null,
    ...
    console.log(`🎯 [${correlationId}] BLOCKS REQUEST: lat=${lat} lng=${lng} userId=${userId} strategy=${!!strategy}`);
