...
/**
 * Run briefing provider, then generate strategy_for_now
 * Simplified pipeline: briefing → strategy
 */
export async function runSimpleStrategyPipeline({ snapshotId, userId, userAddress, city, state, lat, lng, snapshot }) {
  ...
  try {
    ...
    // Run briefing first
    console.log(`[runSimpleStrategyPipeline] 🚀 Running briefing...`);
    try {
      await runBriefing(snapshotId);
      console.log(`[runSimpleStrategyPipeline] ✅ Briefing complete`);
    } catch (briefingErr) {
      console.error(`[runSimpleStrategyPipeline] ❌ Briefing failed:`, briefingErr.message);
      // Continue anyway - strategy can work without briefing
    }

    // Now generate strategy_for_now
    console.log(`[runSimpleStrategyPipeline] 🤖 Generating strategy_for_now...`);
    const { runConsolidator } = await import('../providers/consolidator.js');

    try {
      await runConsolidator(snapshotId);
      console.log(`[runSimpleStrategyPipeline] ✅ Strategy complete`);
    } catch (strategyErr) {
      console.error(`[runSimpleStrategyPipeline] ❌ Strategy generation failed:`, strategyErr.message);

      // Synthesize fallback from briefing
      const [strategyRow] = await db.select().from(strategies)
        .where(eq(strategies.snapshot_id, snapshotId)).limit(1);

      const { synthesizeFallback } = await import('./strategy-utils.js');
      const fallbackStrategy = synthesizeFallback(strategyRow?.briefing);

      await db.update(strategies).set({
        strategy_for_now: fallbackStrategy,
        status: 'ok_partial',
        error_message: `Fallback used: ${strategyErr.message}`,
        updated_at: new Date()
  ...
  try {
    ...
    await db.update(strategies).set({
      ...
      briefing: {
        ...
        events: geminiResult.events ?? [],
        traffic: geminiResult.traffic ?? [],
        holidays: geminiResult.holiday ? [geminiResult.holiday] : []
      },
      strategy_for_now: consolidated.strategy || '',
    ...
    try {
      const { generateEnhancedSmartBlocks } = await import('../jobs/enhanced-smart-blocks.js');
