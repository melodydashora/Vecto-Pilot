...
import { callGPT5WithBudget } from '../llm/gpt5-retry.js';
import { callClaude } from '../adapters/anthropic.js';
import { capturelearning, LEARNING_EVENTS } from '../../middleware/learning-capture.js';
import { indexStrategy } from '../search/semantic-search.js';
...
export async function generateStrategyForSnapshot(snapshot_id) {
  ...
  try {
    ...
    // Fetch briefing data from strategies table
    const [strategyRow] = await db.select().from(strategies)
      .where(eq(strategies.snapshot_id, snapshot_id)).limit(1);

    const briefingData = strategyRow?.briefing || { events: [], holidays: [], traffic: [], news: [] };

    // Format briefing for strategist context
    let briefingContext = '';
    if (briefingData.events?.length > 0) {
      briefingContext += `\nEVENTS:\n${briefingData.events.map(e => `• ${e}`).join('\n')}`;
    }
    if (briefingData.holidays?.length > 0) {
      briefingContext += `\n\nHOLIDAYS:\n${briefingData.holidays.map(h => `• ${h}`).join('\n')}`;
    }
    if (briefingData.traffic?.length > 0) {
      briefingContext += `\n\nTRAFFIC:\n${briefingData.traffic.map(t => `• ${t}`).join('\n')}`;
    }
    if (briefingData.news?.length > 0) {
      briefingContext += `\n\nNEWS:\n${briefingData.news.map(n => `• ${n}`).join('\n')}`;
    }
