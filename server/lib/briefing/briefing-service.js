...
// 2026-05-02: Workstream 6 Step 1 — events pipeline extracted (commit 8/11, the big one).
// fetchEventsForBriefing, deduplicateEvents, filterInvalidEvents re-exported below
// to preserve the public API surface:
//   - server/api/briefing/briefing.js imports filterInvalidEvents (POST /filter-invalid-events)
//   - server/lib/briefing/dump-last-briefing.js imports filterInvalidEvents
//   - refreshEventsInBriefing in this file uses fetchEventsForBriefing via the same binding
//   - deduplicateEvents was originally exported "for use in briefing.js events endpoint"
//     (per inline comment) — kept exported for that surface even though no current external caller
// Four dead code paths deleted in this commit (Option A precedent — see
// claude_memory #294 + #295 + #296): fetchEventsWithClaudeWebSearch,
// _fetchEventsWithGemini3ProPreviewLegacy, mapGeminiEventsToLocalEvents, LocalEventSchema —
// all with zero callers across server/, client/, shared/, scripts/.
// SURVIVAL GUARDRAILS preserved: deduplicateEvents (HASH dedup, Rule 16 — distinct from
// deduplicateEventsSemantic.js); filterInvalidEvents (LIVE compat shim — distinct from
// validateEventsHard).
import { discoverEvents, fetchEventsForBriefing, deduplicateEvents, filterInvalidEvents } from './pipelines/events.js';
