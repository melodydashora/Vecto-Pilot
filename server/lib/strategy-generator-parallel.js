...
export async function runSimpleStrategyPipeline({ snapshotId, userId, userAddress, city, state, lat, lng, snapshot }) {
  try {
    ...
    // NOTE: Briefing is now generated by briefing-service.js when snapshot is created (uses Gemini directly)
    // Skip the old runBriefing provider which used Perplexity (now disabled)
    
    // Run minstrategy provider
    const minResult = await Promise.allSettled([
      runMinStrategy(snapshotId)
    ]);
    
    // Check results
    const minFailed = minResult[0]?.status === 'rejected';
    
    if (minFailed) {
      console.error(`[strategy-pipeline] Minstrategy failed:`, minResult[0].reason?.message || minResult[0].reason);
    }
