...
export async function getEnhancedProjectContext(options = {}) {
  ...
  try {
    ...
    console.warn('[Assistant Enhanced Context] Failed to load recent snapshots:', err.message);
  }
  ...
  try {
    ...
    console.warn('[Assistant Enhanced Context] Failed to load recent strategies:', err.message);
  }
  ...
  try {
    ...
    console.warn('[Assistant Enhanced Context] Failed to load recent actions:', err.message);
  }
  ...
  try {
    const prefs = await memoryQuery({ 
      ...
      scope: "assistant_preferences", 
      userId: null,
      limit: 50 
    });
    context.assistantPreferences = Object.fromEntries(
      prefs.map(p => [p.key, p.content])
    );
  } catch (err) {
    console.warn('[Assistant Enhanced Context] Failed to load preferences:', err.message);
  }
  ...
  try {
    ...
    console.warn('[Assistant Enhanced Context] Failed to load session history:', err.message);
  }
  ...
  try {
    ...
    console.warn('[Assistant Enhanced Context] Failed to load project state:', err.message);
  }
  ...
  try {
    ...
    console.warn('[Assistant Enhanced Context] Failed to load conversation history:', err.message);
  }
...
export async function storeAssistantMemory(title, content, metadata = {}, ttlDays = 730) {
  try {
    ...
    await pool.query(
      `INSERT INTO ${ASSISTANT_TABLE} (session_id, entry_type, title, content, metadata, expires_at)
       VALUES ($1, $2, $3, $4, $5, $6)`,
      [
        'assistant',
        'context',
    ...
    console.warn('[Assistant Enhanced Context] Failed to store assistant memory:', err.message);
    return false;
  }
}
...
export async function getAssistantMemory(userId = null, limit = 50) {
  try {
    const { getSharedPool } = await import("../db/pool.js");
    const pool = getSharedPool();
    if (!pool) return [];
    ...
    console.warn('[Assistant Enhanced Context] Failed to load assistant memory:', err.message);
    return [];
  }
}
