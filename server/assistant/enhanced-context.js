  ...
  try {
    const response = await fetch("https://api.anthropic.com/v1/messages", {
      ...
      body: JSON.stringify({
        ...
        max_tokens: 4096,
        system: "You are a helpful research assistant with access to real-time web information. Provide accurate, up-to-date information with citations from web search results.",
        tools: [
          {
...
export async function storeAgentMemory(title, content, metadata = {}, ttlDays = 730) {
  try {
    ...
    console.warn('[Assistant Enhanced Context] Failed to store agent memory:', err.message);
    return false;
  }
}
...
export async function getAgentMemory(userId = null, limit = 50) {
  try {
    ...
    console.warn('[Assistant Enhanced Context] Failed to load agent memory:', err.message);
    return [];
  }
}
