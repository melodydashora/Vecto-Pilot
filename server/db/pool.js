...
export function getSharedPool() {
  if (!sharedPool) {
    ...
    const config = {
      ...
      max: parseInt(process.env.PG_MAX ?? '20', 10), // Increased for production
      min: parseInt(process.env.PG_MIN ?? '5', 10), // Keep more connections warm
      
      // Idle timeout - 1 minute (adjusted for potential higher load)
      idleTimeoutMillis: parseInt(process.env.PG_IDLE_TIMEOUT_MS ?? '60000', 10),

      // Connection timeout - How long to wait for new connection
      connectionTimeoutMillis: parseInt(process.env.PG_CONNECTION_TIMEOUT_MS ?? '5000', 10), // 5 seconds
