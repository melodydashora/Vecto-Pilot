...
const isCloudRun = !!(
  process.env.K_SERVICE || process.env.CLOUD_RUN_AUTOSCALE === "1"
);
const isAutoscale = isCloudRun && !isReplit; // Cloud Run autoscale, NOT Replit
const fastBoot = process.env.FAST_BOOT === "1" || isAutoscale;

console.log(
  `[env] 🔍 ENABLE_BACKGROUND_WORKER="${process.env.ENABLE_BACKGROUND_WORKER}" (type: ${typeof process.env.ENABLE_BACKGROUND_WORKER})`,
);
console.log(
  `[env] 🔍 isReplit=${isReplit}, isCloudRun=${isCloudRun}, isAutoscale=${isAutoscale}`,
);

if (isAutoscale) {
  console.log(
    "[autoscale] Cloud Run autoscale detected - using fast boot profile",
  );
}
if (isReplit) {
...
(async function main() {
  ...
  const LISTEN_PORT =
    MODE === "split" ? Number(process.env.GATEWAY_PORT || PORT) : PORT;
  ...
  setInterval(() => {
    ...
    if (p95 > 200) {
      console.warn(
        `[perf] ⚠️  Event loop lag p95=${p95}ms — pausing background tasks`,
      );
  ...
  setImmediate(async () => {
    ...
    try {
      const { timeoutMiddleware } = await import(
    ...
    app.use((req, res, next) => {
      ...
      res.on("finish", () => {
        console.log(
          `[gateway] ${req.method} ${req.originalUrl} -> ${res.statusCode} ${Date.now() - t}ms`,
        );
      });
      next();
    });
    ...
    app.get("/diagnostics/memory", (_req, res) =>
      res.json(process.memoryUsage()),
    );
    app.post("/diagnostics/prefs", express.json(), (req, res) =>
      res.json({ ok: true, set: req.body || {} }),
    );
    app.post("/diagnostics/session", express.json(), (req, res) =>
      res.json({ ok: true, phase: req.body?.phase ?? "unknown" }),
    );
    ...
    if (MODE === "mono") {
      ...
      try {
        ...
        mountAgent({
          app,
          basePath: AGENT_PREFIX,
          wsPath: WS_PUBLIC_PATH,
          server,
        });
        console.log(
          `[mono] ✓ Agent routes mounted at ${AGENT_PREFIX}, WS at ${WS_PUBLIC_PATH}`,
        );
      } catch (e) {
      ...
      app.post("/closed-venue-reasoning", (req, res) => {
        console.log(
          "[redirect] /closed-venue-reasoning -> /api/closed-venue-reasoning",
        );
      ...
      app.use(API_PREFIX, (_req, res) =>
        res.status(404).json({ ok: false, error: "NOT_FOUND", mode: "mono" }),
      );
      ...
      if (isDev) {
        ...
        proxy.on("error", (err) =>
          console.error("[mono] Vite proxy error:", err.message),
        );
        ...
        if (CLIENT_DIST) {
          ...
          if (!fs.existsSync(clientDistPath)) {
            console.error(
              "❌ [gateway] Client build not found at:",
              clientDistPath,
            );
            console.error(
              '❌ [gateway] Run "npm run build:client" before starting the server',
            );
          }
          ...
          console.warn(
            "[gateway] No client build found — /app/ will 404 until you build the client.",
          );
        }
      ...
      setImmediate(async () => {
        console.log(
          "[gateway] Background worker managed by separate process (strategy-generator.js)",
        );
        ...
        const initSteps = [
          {
            name: "Job seeding",
            fn: async () => {
              const { seedJobIfEmpty } = await import(
          ...
          },
          {
            ...
            fn: async () => {
              const { safeAssertStrategies } = await import(
          ...
          },
          {
            ...
            fn: async () => {
              if (!fastBoot) {
                const { maybeWarmCaches } = await import(
                  "./server/lib/strategies/assert-safe.js"
                );
        ...
        for (const step of initSteps) {
          ...
          if (globalThis.__PAUSE_BACKGROUND__) {
            console.warn(
              `[boot] Pausing due to event loop lag before: ${step.name}`,
            );
    ...
    //  ═══════════════════════════════════════════════════════════════════
      ...
      app.use(AGENT_PREFIX, (req, res) => {
        req.url =
          req.originalUrl.replace(new RegExp(`^${AGENT_PREFIX}`), "") || "/";
        proxy.web(req, res, { target: agentTarget, changeOrigin: true });
      });

      // 2. SDK routes
      app.use(
        ["/assistant", API_PREFIX, "/diagnostics", SOCKET_IO_PATH],
        (req, res) => {
      ...
      app.use(API_PREFIX, (req, res) => {
        res
          .status(404)
          .json({ error: "not_found", path: req.path, mode: "split" });
      });
      ...
      console.log(
        `🚀 [split] Proxies mounted: agent=${agentTarget}, sdk=${sdkTarget}${isDev ? ", vite=http://127.0.0.1:5173" : ""}`,
      );
      console.log(
        `[split] Root shell retained; health endpoints never shadowed`,
      );
    }
  ...
  server.on("error", (err) => {
    ...
    if (err?.code === "EADDRINUSE") {
      console.error(
        `[gateway] Port ${PORT} in use - trying to continue anyway`,
      );
    }
  });
...
async function prepareDb() {
  ...
  try {
    ...
    if (err.message.includes("vector") || err.message.includes("extension")) {
      console.warn(
        "[gateway] ⚠️ Vector extension unavailable - semantic search disabled",
      );
