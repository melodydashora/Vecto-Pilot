...
| `coach_memos` | 2026-05-12: Coach → Claude Code memo queue. DB-backed for Cloud Run survivability (replaces the prior `fs.appendFile`-only path that died on container redeploys). Workspace pulls new rows into `docs/coach-inbox.md` via `npm run pull-coach-memos`. Schema at `shared/schema.js`; migration `migrations/20260512_coach_memos.sql`. Coach action tag: `[COACH_MEMO]`. Plan: `docs/review-queue/PLAN_coach-memo-db-route-and-workspace-pull-2026-05-12.md`. |
...
| 6 | `docs/coach-inbox.md` | Memos from the Rideshare Coach (Gemini) for Claude Code. **Prod memos arrive via `npm run pull-coach-memos`** (2026-05-12) — DB-backed `coach_memos` table is the source of truth; the file is materialized on-demand by the workspace operator. Run the pull before relying on this file if you suspect new prod activity. |
