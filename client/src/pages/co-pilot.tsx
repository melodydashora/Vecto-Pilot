          ...
          <div data-testid="briefing-section" className="mb-24">
            ...
            <div className="mb-6 flex items-center gap-3 flex-wrap justify-between">
              <div className="flex items-center gap-2">
                ...
                <h2 className="text-lg font-semibold text-gray-800">Rideshare Briefing</h2>
            ...
            <Card className="border border-indigo-200 bg-gradient-to-r from-indigo-50 to-blue-50 mb-6">
              <CardContent className="p-6">
                <div className="space-y-4">
                  <div>
                    <h3 className="font-semibold text-gray-900 text-base mb-2">Today's Summary</h3>
                    <p className="text-gray-700 leading-relaxed">
                      {snapshotData && (
                        <>
                          {snapshotData.city}, {snapshotData.state} • {snapshotData.day_part_key?.charAt(0).toUpperCase() + snapshotData.day_part_key?.slice(1)} • Weather: {snapshotData.weather?.conditions}
                        </>
                      )}
                      {!snapshotData && 'Loading briefing information...'}
                    </p>
                  </div>
                  
                  {persistentStrategy && (
                    <div>
                      <h3 className="font-semibold text-gray-900 text-base mb-2">AI Strategy</h3>
                      <p className="text-gray-700 leading-relaxed text-sm">
                        {persistentStrategy.substring(0, 300)}
                        {persistentStrategy.length > 300 ? '...' : ''}
                      </p>
                    </div>
                  )}
                  ...
                  {blocks.length > 0 && (
                    <div>
                      <h3 className="font-semibold text-gray-900 text-base mb-2">Top Recommendations</h3>
                      <div className="space-y-2">
                        {blocks.slice(0, 3).map((block, idx) => (
                          <div key={idx} className="flex items-start gap-3 p-2 rounded-lg bg-white/60 border border-indigo-100">
                            <Badge className="bg-blue-100 text-blue-700 text-xs mt-0.5">#{idx + 1}</Badge>
                            <div className="flex-1 min-w-0">
                              <p className="font-medium text-gray-800 truncate">{block.name}</p>
                              <p className="text-xs text-gray-600">{block.address}</p>
                            </div>
                            {block.estimated_earnings && (
                              <div className="text-right">
                                <p className="text-sm font-semibold text-green-600">${block.estimated_earnings}</p>
                              </div>
                            )}
                          </div>
                        ))}
            ...
            <div className="mb-6">
              ...
              <CoachChat
                userId={snapshotData?.user_id || 'default'}
                snapshotId={lastSnapshotId || undefined}
                strategyId={blocksData?.ranking_id || undefined}
                strategy={persistentStrategy || undefined}
                snapshot={snapshotData}
                blocks={blocks}
                strategyReady={!!persistentStrategy}
              />
