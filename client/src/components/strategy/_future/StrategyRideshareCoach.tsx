...
export function StrategyCoach({ snapshotId }: { snapshotId?: string }) {
  ...
  return (
    <Card className="bg-gradient-to-br from-purple-50 via-blue-50 to-indigo-50 border-purple-300 overflow-hidden" data-testid="strategy-coach-card">
      <CardContent className="p-4">
...
function CoachBody({ min, consolidated }: { min?: string; consolidated?: string }) {
  ...
  if (text.trim().startsWith('{')) {
    try {
      ...
      return (
        <pre className="text-sm text-gray-700 whitespace-pre-wrap font-sans break-words overflow-x-auto max-w-full">
  ...
  return (
    <div
      className="text-sm text-gray-700 leading-relaxed whitespace-pre-wrap break-words overflow-x-auto max-w-full prose prose-sm prose-gray max-h-[500px] overflow-y-auto"
      data-testid="strategy-coach-text"
    >
      {text}
    </div>
  );
