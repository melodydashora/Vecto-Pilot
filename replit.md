...
- **Strategy Split Architecture (NEW)**: Consolidated Gemini strategy now splits into two tab-specific strategies:
  ...
  - **Right Now Strategy** (Strategy Tab): Immediate tactical guidance generated by GPT-5.2 using consolidated output + location. Focused on "next 1 hour" actions for maximum earnings. Uses `reasoning.effort: "medium"` per model-registry.js specs.
