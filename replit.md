...
### Deployment: Green Blocks with Audit Trail (2025-10-31)
...
- Source tagging: `source: 'snapshot'` (header-resolved coords)
- Reverse geocode attribution: `reverseGeo: formatted_address || city`
- Audit logged: `{coords, source, reverseGeo, snapshotId}`

#### Audit Trail Structure
All POST `/api/blocks/fast` responses include `audit` array with timestamped entries:
1. **input**: Origin normalization (`{coords, source, reverseGeo}`)
2. **generation**: GPT-5 venue generation (`{venue_count, coords_count, generation_ms, model}`)
3. **enrichment**: Drive time enrichment (`{total, with_drive_time, with_hours, enrichment_ms}`)
4. **status**: Block quality distribution (`{green, yellow, red, total}`)
5. **events**: Event matching status (`{event_match, reason}`)

#### Block Status System
**Flags** (4 checks per block):
- `coordsOk`: Location coordinates valid (`lat && lng`)
- `stagingOk`: Staging area coordinates valid (`staging_lat && staging_lng`)
- `tipsOk`: Pro tips generated by GPT-5 (`pro_tips`)
- `enrichmentOk`: Drive time enrichment successful (`driveTimeMinutes > 0`)

**Status Levels**:
- **Green**: 4/4 flags pass
- **Yellow**: 2-3/4 flags pass
- **Red**: 0-1/4 flags pass

**Reason Codes** (yellow/red blocks):
- `coords_missing`: Location coordinates invalid
- `staging_missing`: Staging area coordinates invalid
- `tips_missing`: No pro_tips generated
- `enrichment_incomplete`: Drive time enrichment failed

#### Business Hours Attribution
**Source**: Google Places API (new) or stored `business_hours` JSONB field
**Guards**: Safe navigation (`?.`) prevents crashes on missing hours
**UI Display**: Shows hours when available, otherwise null (no crash)
**Audit**: `hours_provider=<provider>` when available

#### ~~Event-to-Venue Matching (Route Distance ≤2 Miles)~~
**Status**: BLOCKED - Not implemented (2025-10-31)
...
- Route proximity: Route distance ≤2 miles from origin to venue anchor
- Audit: `event_match=direct|route|none, route_distance_miles=<value>`

**Current Behavior**: 
- `eventBadge`: Always null (no venue_events data)
- `eventSummary`: Always null (no venue_events data)
...
**Next Action**: Create `venue_events` table and implement route-based matching logic when event data source is available
